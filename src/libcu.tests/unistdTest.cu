...
static __global__ void g_unistd_test1()
{
	...
	/* Host Absolute */
	int i0a = chdir(HostDir"test.txt"); assert(i0a == 0);

	/* Device Absolute */
	int j0a = chdir(DeviceDir"test.txt"); assert(j0a == 0);
	...
	/* Host Absolute */
	int k0_fd = 0;
	int k0a = dup(k0_fd); assert(k0a == 0);
	int k1a = dup2(k0_fd, k0_fd); assert(k0a == 0);

	/* Device Absolute */
	int l0a = dup(k0_fd); assert(l0a == 0);
	int l1a = dup2(k0_fd, k0_fd); assert(l0a == 0);
	...
	/* Host Absolute */
	int o0a = rmdir(HostDir"test"); assert(o0a == 0);

	/* Device Absolute */
	int p0a = rmdir(DeviceDir"test"); assert(p0a == 0);
}
