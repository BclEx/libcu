...
static __global__ void g_dirent_test1()
{
	...
	/* Host Absolute */
	DIR *a0a = opendir("C:\\missing"); int a0b = closedir(a0a); assert(!a0a && a0b == -1);
	mkdir("C:\\test", 0); mkdir("C:\\test\\dir0", 0);
	DIR *a1a = opendir("C:\\test"); bool a1b = !strcmp(a1a->ent.d_name, "test"); testReading(a0a); int a1c = closedir(a1a); assert(a1a && a1b && !a1c);

	/* Device Absolute */
	DIR *b0a = opendir(":\\missing"); int b0b = closedir(b0a); assert(!b0a && b0b == -1);
	mkdir(":\\test", 0); mkdir(":\\test\\dir0", 0);
	DIR *b1a = opendir(":\\test"); bool b1b = !strcmp(b1a->ent.d_name, "test"); testReading(b0a); int b1c = closedir(b1a); assert(b1a && b1b && !b1c);
