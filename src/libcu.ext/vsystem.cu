...
*/
__host_device__ void vsys_close(vsystemfile *p) { if (p->methods) { p->methods->close(p); p->methods = nullptr; } }
__host_device__ RC vsys_read(vsystemfile *p, void *buf, int amount, int64_t offset) { DO_OS_MALLOC_TEST(p); return p->methods->read(p, buf, amount, offset); }
__host_device__ RC vsys_write(vsystemfile *p, const void *buf, int amount, int64_t offset){ DO_OS_MALLOC_TEST(p); return p->methods->write(p, buf, amount, offset); }
__host_device__ RC vsys_truncate(vsystemfile *p, int64_t size) { return p->methods->truncate(p, size); }
__host_device__ RC vsys_sync(vsystemfile *p, int flags) { DO_OS_MALLOC_TEST(id); return flags ? p->methods->sync(p, flags) : RC_OK; }
__host_device__ RC vsys_fileSize(vsystemfile *p, int64_t *size) { DO_OS_MALLOC_TEST(p); return p->methods->fileSize(p, size); }
__host_device__ RC vsys_lock(vsystemfile *p, int lockType) { DO_OS_MALLOC_TEST(p); return p->methods->lock(p, lockType); }
__host_device__ RC vsys_unlock(vsystemfile *p, int lockType) { return p->methods->unlock(p, lockType); }
__host_device__ RC vsys_checkReservedLock(vsystemfile *p, int *res) { DO_OS_MALLOC_TEST(id); return p->methods->checkReservedLock(p, res); }
...
*/
__host_device__ RC vsys_fileControl(vsystemfile *p, int op, void *arg)
{
#ifdef _TEST
...
__host_device__ void vsys_fileControlHint(vsystemfile *p, int op, void *arg) { p->methods->fileControl(p, op, arg); }
__host_device__ int vsys_sectorSize(vsystemfile *p) { int (*sectorSize)(vsystemfile *) = p->methods->sectorSize; return sectorSize ? sectorSize(p) : LIBCU_DEFAULT_SECTOR_SIZE; }
__host_device__ int vsys_deviceCharacteristics(vsystemfile *p) { return p->methods->deviceCharacteristics(p); }
#ifndef NO_WAL
__host_device__ RC vsys_shmLock(vsystemfile *p, int offset, int n, int flags){ return p->methods->shmLock(p, offset, n, flags); }
__host_device__ void vsys_shmBarrier(vsystemfile *p) { p->methods->shmBarrier(p); }
__host_device__ RC vsys_shmUnmap(vsystemfile *p, int deleteFlag) { return p->methods->shmUnmap(p, deleteFlag); }
__host_device__ RC vsys_shmMap(vsystemfile *p, int pageId, int pageSize, int extend, void volatile **pages) { DO_OS_MALLOC_TEST(p); return p->methods->shmMap(p, pageId, pageSize, extend, pages); }
#endif

#if LIBCU_MAX_MMAP_SIZE > 0
/* The real implementation of xFetch and xUnfetch */
__host_device__ RC vsys_fetch(vsystemfile *p, int64_t offset, int amount, void **pages) { DO_OS_MALLOC_TEST(p); return p->methods->fetch(p, offset, amount, pages); }
__host_device__ RC vsys_unfetch(vsystemfile *p, int64_t offset, void *pages) { return p->methods->unfetch(p, offset, pages); }
#else
/* No-op stubs to use when memory-mapped I/O is disabled */
__host_device__ RC vsys_fetch(vsystemfile *p, int64_t offset, int amount, void **pages){ *pages = nullptr; return RC_OK; }
__host_device__ RC vsys_unfetch(vsystemfile *p, int64_t offset, void *pages) { return RC_OK; }
#endif

/* The next group of routines are convenience wrappers around the VFS methods. */
__host_device__ RC vsys_open(vsystem *p, const char *path, vsystemfile *file, int flags, int *flagsOut) { DO_OS_MALLOC_TEST(nullptr); RC rc = p->open(p, path, file, flags & 0x87f7f, flagsOut); assert(rc == RC_OK || !file->methods); return rc; }
...
__host_device__ RC vsys_delete(vsystem *p, const char *path, int dirSync) { DO_OS_MALLOC_TEST(nullptr); assert(dirSync == 0 || dirSync == 1); return p->delete_(p, path, dirSync); }
__host_device__ RC vsys_access(vsystem *p, const char *path, int flags, int *resOut) { DO_OS_MALLOC_TEST(nullptr); return p->access(p, path, flags, resOut); }
__host_device__ RC vsys_fullPathname(vsystem *p, const char *path, int pathOutSize, char *pathOut) { DO_OS_MALLOC_TEST(0); pathOut[0] = 0; return p->fullPathname(p, path, pathOutSize, pathOut); }
#ifndef NO_LOAD_EXTENSION
__host_device__ void *vsys_dlOpen(vsystem *p, const char *path) { return p->dlOpen(p, path); }
__host_device__ void vsys_dlError(vsystem *p, int bytes, char *bufOut) { p->dlError(p, bytes, bufOut); }
__host_device__ void (*vsys_dlSym(vsystem *p, void *handle, const char *sym))() { return p->dlSym(p, handle, sym); }
__host_device__ void vsys_dlClose(vsystem *p, void *handle) { p->dlClose(p, handle); }
#endif
__host_device__ RC vsys_Randomness(vsystem *p, int bytes, char *bufOut) { return p->randomness(p, bytes, bufOut); }
__host_device__ RC vsys_Sleep(vsystem *p, int microseconds) { return p->sleep(p, microseconds); }
__host_device__ int vsys_GetLastError(vsystem *p) { return p->getLastError ? p->getLastError(p, 0, 0) : 0; }
__host_device__ RC vsys_CurrentTimeInt64(vsystem *p, int64_t *timeOut) { if (p->version >= 2 && p->currentTimeInt64) return p->currentTimeInt64(p, timeOut); double r; RC rc = p->currentTime(p, &r); *timeOut = (int64_t)(r*86400000.0); return rc; }
...
*/
__host_device__ RC vsys_openMalloc(vsystem *p, const char *fileName, vsystemfile **file, int flags, int *flagsOut)
{
...
__host_device__ void vsys_closeAndFree(vsystemfile *p) { assert(p); vsys_close(p); mfree(p); }
...
*/
__host_device__ RC vsystemFakeInit()
{
	//void *p = alloc(10);
...
__host_device__ vsystem *vsystemFind(const char *name)
{
...
__host_device__ RC vsystemRegister(vsystem *p, int makeDefault)
{
#ifndef OMIT_AUTOINIT
	RC rc = runtimeInitialize();
	if (rc) return rc;
...
__host_device__ RC vsystemUnregister(vsystem *p)
{
#if LIBCU_THREADSAFE
	mutex *mutex = mutex_alloc(MUTEX_STATIC_MASTER);
#endif
