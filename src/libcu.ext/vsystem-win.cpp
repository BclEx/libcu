...
static RC winOpen(vsystem *sys, const char *name, vsystemfile *id, int flags, int *outFlags)
{
	...
	if (!utf8Name) {
		...
		rc = winGetTempname(sys, &zTmpname);
		if (rc != RC_OK) {
...
static RC winFullPathname(vsystem *sys, const char *relative, int fullLength, char *full)
{
	...
	*/
	if (_libcu_data_directory && !winIsVerbatimPathname(relative))snprintf(full, MIN(fullLength, sys->maxPathname), full, "%s%c%s", _libcu_data_directory, winGetDirSep(), relative);
	else snprintf(full, MIN(fullLenth, sys->maxPathname), full, "%s", relative);
	return RC_OK;
#endif
	...
	if (_libcu_data_directory && !winIsVerbatimPathname(relative)) {
		...
		**       for converting the relative path name to an absolute one by prepending the data directory and a backslash.
		*/
		snprintf(full, MIN(fullLength, sys->maxPathname), full, "%s%c%s", _libcu_data_directory, winGetDirSep(), relative);
		return RC_OK;
	}
...
#endif
	if (out) {
		snprintf(full, MIN(fullLength, sys->maxPathname), full, "%s", out);
		mfree(out);
		return RC_OK;
	}
...
#endif
	*nNow = winFiletimeEpoch + ((((int64_t)ft.dwHighDateTime)*max32BitValue) + (int64_t)ft.dwLowDateTime)/(int64_t)10000;
#ifdef LIBCU_TEST
