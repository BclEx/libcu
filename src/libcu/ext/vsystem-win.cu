...
#  define LIBCU_WIN32_HEAP_INIT_SIZE   ((LIBCU_WIN32_CACHE_SIZE) * \
	(LIBCU_DEFAULT_PAGE_SIZE) + \
	(LIBCU_WIN32_HEAP_INIT_EXTRA))
#endif
...
*/
#ifdef LIBCU_TEST
LONG LIBCU_WIN32_VOLATILE sqlite3_os_type = 0;
#else
static LONG LIBCU_WIN32_VOLATILE sqlite3_os_type = 0;
#endif
...
public:
	enum WINFILE : uint8
	{
		...
		WINFILE_PSOW = 0x10,		// LIBCU_IOCAP_POWERSAFE_OVERWRITE
	};
...
void *WinVAlloc::Realloc(void *prior, int bytes)
{
	...
	_assert(heap != NULL);
	_assert(heap != INVALID_HANDLE_VALUE);
#if !OS_WINRT && defined(LIBCU_WIN32_MALLOC_VALIDATE)
	_assert(osHeapValidate(heap, WIN32_HEAP_FLAGS, prior));
#endif
	_assert(bytes >= 0);
	void *p;
...
RC WinVFile::Sync(SYNC flags)
{
	...
	g_sync_count++;
#endif
#ifdef NO_SYNC // If we compiled with the LIBCU_NO_SYNC flag, then syncing is a no-op
	return RC_OK;
#else
...
int WinVFile::ShmMap(int region, int sizeRegion, bool isWrite, void volatile **pp)
{
	...
	if (shmNode->RegionLength <= region)
	{
		...
		if (size < bytes)
		{
			// The requested memory region does not exist. If isWrite is set to zero, exit early. *pp will be set to NULL and LIBCU_OK returned.
...
#endif
#endif
	// Check that the output buffer is large enough for the temporary file name. If it is not, return LIBCU_ERROR.
...
{
	// 0x87f7f is a mask of LIBCU_OPEN_ flags that are valid to be passed down into the VFS layer.  Some LIBCU_OPEN_ flags (for example,
	// LIBCU_OPEN_FULLMUTEX or LIBCU_OPEN_SHAREDCACHE) are blocked before reaching the VFS.
	...
	// LIBCU_OPEN_EXCLUSIVE is used to make sure that a new file is created. SQLite doesn't use it to indicate "exclusive access" as it is usually understood.
...
RC WinVSystem::Access(const char *filename, ACCESS flags, int *resOut)
{
	...
	if (isNT())
	{
		...
		if (rc)
		{
			// For an LIBCU_ACCESS_EXISTS query, treat a zero-length file as if it does not exist.
